class EpubGenerator {
  ...
  async generate(logCallback = console.log) {
    ...
    // Helper function to safely get authors
    function getAuthors(metadata) {
      if (!metadata || !metadata.author) return 'Unknown Author';
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    ...
    if (this.novelData.metadata && Array.isArray(this.novelData.metadata.otherworks)) {
...
<body style="margin:0; text-align:center;">
  ...
  <p><strong>Author:</strong> ${escapeXML(getAuthors(this.novelData.metadata))}</p>
</body></html>`;
    ...
    /* 4. Information page */
    const authors = getAuthors(this.novelData.metadata);
    const genres = Array.isArray(this.novelData.metadata.genres) ? this.novelData.metadata.genres : [];
...
<body>
  <h1>${escapeXML(this.novelData.metadata.title)}</h1>
  <p><strong>Author:</strong> ${escapeXML(authors)}</p>
  <p><strong>Status:</strong> ${escapeXML(this.novelData.metadata.status || 'Unknown')}</p>
  ...
  ${genres.length ? `<p><strong>Genres:</strong> ${genres.map(escapeHTMLContent).join(', ')}</p>` : ''} <!-- UPDATED -->

  <h3>Description</h3>
  <p>${escapeHTMLContent(this.novelData.metadata.description || '')}</p>

${this.novelData.metadata.otherworks && Array.isArray(this.novelData.metadata.otherworks) && this.novelData.metadata.otherworks.length ? `
  <h3>Other Works by ${escapeXML(authors)}</h3>
      ...
      <li>
        <div style="display: flex; align-items: flex-start;flex-direction:column;">
          ...
          <div>
            ...
            ${work.genres && Array.isArray(work.genres) && work.genres.length ? `<em>${work.genres.map(escapeHTMLContent).join(', ')}</em>` : ''} <!-- UPDATED -->
          </div>
        </div>
        ${work.description 
...
<body>
  ...
  <nav epub:type="toc" id="toc">
    <ol>
      ${coverFileName ? '<li><a href="cover.xhtml">Cover</a></li>' : ''}
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="BookId" version="2.0">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    ...
    <dc:creator>${escapeHTMLContent(authors)}</dc:creator> <!-- UPDATED -->
    <dc:language>en</dc:language>
${genres.map(genre => `<dc:subject>${escapeHTMLContent(genre)}</dc:subject>`).join('\n')}
<dc:identifier id="BookId">urn:uuid:${crypto.randomUUID()}</dc:identifier>
  ...
  ${coverFileName ? '<item id="cover-page" href="cover.xhtml" media-type="application/xhtml+xml"/>' : ''}
