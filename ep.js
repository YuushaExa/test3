...
<body>
  ...
  ${this.novelData.metadata.altitile ? `<p><strong>Alternative Title:</strong> ${
    Array.isArray(this.novelData.metadata.altitile)
      ? this.novelData.metadata.altitile.map(escapeHTMLContent).join(', ')
      : escapeHTMLContent(this.novelData.metadata.altitile)
  }</p>` : ''}
    ...
    ${this.novelData.metadata.otherworks.map((work, index) => `
      <li>
        <div style="display: flex; align-items: flex-start;flex-direction:column;">
          ${work.cover 
            ? `<img src="${escapeXML(work.cover)}" alt="${escapeHTMLContent(work.title)}" style="margin-right: 15px;" />`
            : '<div style="margin-right: 15px;"></div>'}
          <div>
            <strong><a href="${escapeXML(work.url)}">${escapeHTMLContent(work.title)}</a></strong><br />
            ${work.genres && work.genres.length ? `<em>${work.genres.map(escapeHTMLContent).join(', ')}</em>` : ''}
          </div>
        </div>
        ${work.description 
...
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
  <title>${escapeHTMLContent(ch.title)}</title>
  <meta charset="utf-8"/>
</head>
<body>
  <h1>${escapeHTMLContent(ch.title)}</h1>
  ${processedContent}
</body>
</html>`;
...
<body>
  ...
  <nav epub:type="toc" id="toc">
    <ol>
      ${toc.map(t => `<li><a href="${escapeXML(t.href)}">${escapeHTMLContent(t.title)}</a></li>`).join('\n      ')}
    </ol>
  </nav>
</body>
</html>`;
...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="BookId" version="2.0">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    <dc:title>${escapeHTMLContent(this.novelData.metadata.title)}</dc:title>
    <dc:creator>${this.novelData.metadata.author.map(escapeHTMLContent).join(', ')}</dc:creator>
    <dc:language>en</dc:language>
${this.novelData.metadata.genres.map(genre => `<dc:subject>${escapeHTMLContent(genre)}</dc:subject>`).join('\n')}
<dc:identifier id="BookId">urn:uuid:${crypto.randomUUID()}</dc:identifier>
    <dc:description>${escapeHTMLContent(this.novelData.metadata.description || '')}</dc:description>
